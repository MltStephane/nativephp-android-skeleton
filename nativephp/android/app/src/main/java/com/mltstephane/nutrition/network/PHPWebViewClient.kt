package com.mltstephane.nutrition.network
...
import com.mltstephane.nutrition.security.LaravelSecurity
        ...
        if (statusCode in 300..399) {
            ...
            if (!location.isNullOrEmpty()) {
                ...
                if (currentPath == targetPath) {
                    Log.w(TAG, "🛑 Ignoring self-redirect to same path: $targetPath")
                    return WebResourceResponse("text/html", "UTF-8", 204, "No Content", emptyMap(), ByteArrayInputStream("".toByteArray()))
                }
